---
description: "Generate a comprehensive quarterly report with narrative, data evidence, and an HTML file. Usage: /qc-report or /qc-report Q3 2025"
---
...
If the user specified a quarter (e.g., `/qc-report Q3 2025`), use that.
Otherwise, calculate from today's date using the ranges in `CLAUDE.md`.
...
> "Config not found. Run `/qc-setup` first to configure GitHub, Jira, and your profile."
> **STOP.**
...
```bash
gh search prs --author={github.author} --repo={owner}/{repo} --merged={START}..{END} --json number,title,url,mergedAt --limit 50
```
...
**C. Jira Epics** — use `mcp__atlassian__searchJiraIssuesUsingJql`:
```sql
...
**D. Jira Tickets** — use `mcp__atlassian__searchJiraIssuesUsingJql`:
```sql
